...
export function registerFundingHistoryTool(server: FastMCP): void {
  server.addTool({
    ...
    execute: async (args) => {
      try {
        const context = withObjective(args.context, "funding");
        ...
        return buildResponse({
          ...
          metadata: {
            note: events.length === 0 ? "No explicit funding round mentions found in retrieved sources." : undefined,
          },
        });
      } catch (err) {
